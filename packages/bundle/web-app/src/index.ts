...
export function apply(ctx: Context, config: Config): void {
  ...
  if (config.printUrl || handoffBrowser) {
    ...
    const announceReady = (): void => {
      ...
      if (config.printUrl) {
        console.log(`xyz web: ${webUrl}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
      }
      if (handoffBrowser) {
        console.log('xyz web: opening the default browser; pass --no-open to disable')
