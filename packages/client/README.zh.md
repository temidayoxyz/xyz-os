...
| [`ui-renderer/`](ui-renderer/README.zh.md) | 将 slot 数据绑定到 React，并在客户端启动完成后挂载组装好的应用。 |
...
| [`ui-brand-official/`](ui-brand-official/README.zh.md) | 用官方名称与标志填充通用浏览器品牌 slot。 |
...
| [`ui-reference/`](ui-reference/README.zh.md) | 统一的 Web `@file` / `@session` 引用来源。 |
...
| [`ui-deep-tariff/`](ui-deep-tariff/README.zh.md) | 在输入框下方显示 DeepSeek 高峰/非高峰时段与单价。 |
