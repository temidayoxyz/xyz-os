...
function mount(
  ...
  const renderSlot = ((key: string, owner: object, opts?: { only?: string; fallback?: ReactNode }) => {
    ...
    if (key === 'conversation.hero.brand.mark') { return opts?.fallback ?? null }
...
describe('Hero chrome', () => {
  it('renders the XYZ logo mark through the brand-mark slot', () => {
    const renderSlot = vi.fn<HeroShellProps['renderSlot']>(
      (_name, _owner, opts) => opts?.fallback ?? null,
    )
    const view = render(<HeroShell t={makeTranslate(en, commonEn)} renderSlot={renderSlot} />)
    expect(view.container.querySelector('img[src="/favicon.png"]')).toBeTruthy()
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: same textarea, hero chrome, no header, picker switches the workspace', () => {
    ...
    expect(b.view.container.querySelector('img[src="/favicon.png"]')).toBeTruthy()
